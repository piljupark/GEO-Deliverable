...
def generate_robots_txt(site_url, allow_all_ai=True, sitemap_url=None):
    ...
    lines = [f"# {domain} — AI 크롤러 노출 설정", "# 자동 생성됨 (필요시 직접 수정)", ""]
...
def generate_llms_txt(tech, brand_name=None):
    ...
    lines.append("")
    lines.append("<!-- 크롤 데이터를 기반으로 자동 생성됨. 페이지가 늘어나면 섹션을 추가하세요. -->")
    return "\n".join(lines)
...
def generate_json_ld(tech, brand_name=None, social_urls=None):
    ...
    org = {"@type": "Organization", "@id": org_id, "name": name, "url": tech["url"]}
    if tech.get("meta_desc"):
        org["description"] = tech["meta_desc"]
    if social_urls:
        org["sameAs"] = social_urls
    website = {
        "@type": "WebSite", "@id": f"{tech['url'].rstrip('/')}/#website",
        "url": tech["url"], "name": name, "publisher": {"@id": org_id},
    }
